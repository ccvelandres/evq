...
static inline unsigned int cyg_get_thread_id(void) { return (unsigned int)pxCurrentTCB; }
#else
static inline unsigned int cyg_get_thread_id(void) { return 0; }
#endif
#define US_PER_TICK 10
static volatile uint32_t   __cyg_timestamp; // Incremented by profiler timebase
static inline unsigned int cyg_get_timestamp(void) { return __cyg_timestamp; }

static inline unsigned int cyg_get_active_isr(void)
{
...
bool cyg_profiler_flush(void *ptr, uint16_t len);
void cyg_profiler_init_flush();
void cyg_profiler_store(unsigned int is_enter, void *this_fn, void *call_site);